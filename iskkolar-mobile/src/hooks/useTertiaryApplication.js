  ...
  const getStep1PreflightErrors = useCallback((values, dynamicFamilyMembers, scholarBirth) => {
    ...
    const checkMember = (name, status, occ, inc, prefix, niceName, validateAddress) => {
      ...
      if (status !== "Deceased" && validateAddress) {
        if (!values[prefix + "Street"] || values[prefix + "Street"].trim() === "") errors[prefix + "Street"] = "Street/Unit is required.";
        if (!values[prefix + "Province"] || values[prefix + "Province"].trim() === "") errors[prefix + "Province"] = "Province is required.";
        if (!values[prefix + "City"] || values[prefix + "City"].trim() === "") errors[prefix + "City"] = "City/Municipality is required.";
        if (!values[prefix + "Barangay"] || values[prefix + "Barangay"].trim() === "") errors[prefix + "Barangay"] = "Barangay is required.";
        if (!values[prefix + "Zip"] || values[prefix + "Zip"].trim() === "") {
          errors[prefix + "Zip"] = "Zip Code is required.";
        } else if (!/^\d{4}$/.test(values[prefix + "Zip"])) {
          errors[prefix + "Zip"] = "Zip Code must contain exactly 4 digits.";
        }
      }
    ...
    if (isFatherStarted && values.fatherStatus !== "Deceased") {
      checkMember(values.fatherName, values.fatherStatus, values.fatherOccupation, values.fatherIncome, "father", "Father's", validateFatherAddress);
    ...
    if (isMotherStarted && values.motherStatus !== "Deceased") {
      checkMember(values.motherName, values.motherStatus, values.motherOccupation, values.motherIncome, "mother", "Mother's", validateMotherAddress);
